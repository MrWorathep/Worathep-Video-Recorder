...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    if (isIosDevice && mediaBlob) {
      toast.error(
        "iPhone/iPad ไม่รองรับเล่นไฟล์วิดีโอแบบ .webm กรุณาดาวน์โหลดแล้วเปิดด้วยแอปอื่น"
      );
    }
  }, [isIosDevice, mediaBlob]);
        ...
        <div className="flex flex-col items-center mt-8">
          {!isIosDevice ? (
            ...
            <p className="text-red-600 text-center max-w-md">
              ไฟล์วิดีโอ .webm ไม่รองรับบน iPhone/iPad
              กรุณาดาวน์โหลดแล้วเปิดด้วยแอปอื่น
            </p>
          )}
